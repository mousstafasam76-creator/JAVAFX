...
public class ClientsListView extends FrontView {
    ...
    protected Node createContent() {
        ...
        addBtn.setStyle("-fx-background-color: " + PRIMARY + "; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 10 20; -fx-background-radius: 12; -fx-cursor: hand; -fx-font-size: 13px;");
        addBtn.setOnMouseEntered(e -> addBtn.setStyle("-fx-background-color: " + PRIMARY_DARK + "; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 10 20; -fx-background-radius: 12; -fx-cursor: hand; -fx-font-size: 13px;"));
        addBtn.setOnMouseExited(e -> addBtn.setStyle("-fx-background-color: " + PRIMARY + "; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 10 20; -fx-background-radius: 12; -fx-cursor: hand; -fx-font-size: 13px;"));
        addBtn.setOnAction(e -> navigationManager.navigateTo("addClient"));
    ...
    private VBox createStatCard(String label, Label valueLabel, String color) {
        ...
        card.setStyle("-fx-background-color: white; -fx-background-radius: 16; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.05), 8, 0, 0, 2); -fx-border-color: " + BORDER + "; -fx-border-radius: 16;");
    ...
    private HBox createFilterBar() {
        ...
        bar.setStyle("-fx-background-color: white; -fx-background-radius: 16; -fx-border-color: " + BORDER + "; -fx-border-radius: 16;");
    ...
    private String orderBtnStyle() {
        return "-fx-background-color: white; -fx-border-color: " + BORDER + "; -fx-border-radius: 10; -fx-background-radius: 10; -fx-padding: 8 16; -fx-font-size: 12px; -fx-cursor: hand;";
    }
    private String activeOrderStyle() {
        return "-fx-background-color: " + PRIMARY + "; -fx-text-fill: white; -fx-border-color: " + PRIMARY + "; -fx-border-radius: 10; -fx-background-radius: 10; -fx-padding: 8 16; -fx-font-size: 12px; -fx-cursor: hand;";
    }
    private Button createOrderBtn(String text) {
        Button btn = new Button(text);
        btn.setStyle(orderBtnStyle());
        btn.setOnMouseEntered(e -> { if (!btn.getStyle().contains(PRIMARY)) btn.setStyle(activeOrderStyle()); });
        btn.setOnMouseExited(e -> { if (!btn.getStyle().contains(PRIMARY) || btn.getStyle().contains("white")) btn.setStyle(orderBtnStyle()); });
        return btn;
    }

    private TableView<Client> createTable() {
        TableView<Client> table = new TableView<>();
        table.setStyle("-fx-background-color: white; -fx-background-radius: 16; -fx-border-color: " + BORDER + "; -fx-border-radius: 16;");
    ...
    c.getValue().getPrenom() + " " + c.getValue().getNomc()
));
        ...
        colCmds.setCellFactory(col -> new TableCell<Client, Integer>() {
            @Override protected void updateItem(Integer v, boolean empty) {
                super.updateItem(v, empty);
                if (empty || v == null) setGraphic(null);
                else {
                    Label badge = new Label(String.valueOf(v));
                    badge.setStyle("-fx-background-color: #e2e8f0; -fx-text-fill: #1e293b; -fx-padding: 3 10; -fx-background-radius: 20; -fx-font-size: 12px;");
                    setGraphic(badge);
                    setText(null);
        ...
        colActions.setCellFactory(col -> new TableCell<Client, Void>() {
            ...
            @Override protected void updateItem(Void v, boolean empty) {
                ...
                viewBtn.setOnAction(e -> navigationManager.navigateTo("viewClient?id=" + c.getId()));
                editBtn.setOnAction(e -> navigationManager.navigateTo("editClient?id=" + c.getId()));
                delBtn.setOnAction(e -> {
                    if (AlertUtils.confirmDelete("Supprimer " + c.getNomComplet() + " ?")) {
                        if (controller != null) controller.deleteClient(c);
                        // Le rafraîchissement est automatique car la liste est observable
    ...
    private Button createActionBtn(String icon, String color, String tooltip) {
        ...
        String base = "-fx-background-color: white; -fx-border-color: " + BORDER + "; -fx-border-radius: 10; -fx-background-radius: 10; -fx-padding: 5 9; -fx-font-size: 13px; -fx-cursor: hand;";
        String hover = "-fx-background-color: " + color + "; -fx-text-fill: white; -fx-border-color: " + color + "; -fx-border-radius: 10; -fx-background-radius: 10; -fx-padding: 5 9; -fx-font-size: 13px; -fx-cursor: hand;";
        btn.setStyle(base);
    ...
    if (controller != null) {
        data = controller.getClients(); // Récupère la liste observable du contrôleur
    } else {
        data = FXCollections.observableArrayList();
    }
    // Supprime complètement les appels à new Client(...) car le contrôleur a déjà des exemples
    ...
    private void refreshStats() {
        ObservableList<Client> visible = filteredList != null
            ...
            : (controller != null ? controller.getClients() : FXCollections.observableArrayList());
